# TEMPLATE 05 — Clínica / Consultorio

## Ghost Business
- **Nombre:** DentalCare Facatativá
- **Tipo:** Clínica odontológica
- **Ubicación:** Cra 4 #8-30, Facatativá, Cundinamarca
- **Teléfono:** 314 567 8901
- **WhatsApp:** 3145678901
- **Eslogan:** "Tu sonrisa, nuestra prioridad"

## Identidad Visual
- **Estilo:** Blob Morphing + Suave + Limpio
- **Animación Firma:** Formas orgánicas animadas (blobs) en el fondo, tarjetas con slide 3D
- **Paleta:** Blanco puro, Azul médico (`#1976d2`), Celeste suave, Verde menta
- **Tipografía:** Montserrat (títulos y cuerpo, 400–900)
- **Sensación:** Limpio, profesional, calmado, confiable

## Secciones
1. Header glass limpio
2. Hero con blob decorativo animado
3. Servicios con slide 3D
4. Equipo médico
5. Instalaciones / tecnología
6. Testimonios
7. Ubicación + contacto

## Estado
- [ ] Por desarrollar
